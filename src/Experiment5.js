...
function Experiment5({keyword, columns, wordList, k, n, start, setGridHighlight, setGridHighlightOverride}) {  
  const go = () => {
    search(columns, keyword, doSearchKeywordFuzzy, setGridHighlight, setGridHighlightOverride, true, true);
  }
  const go2 = () => {
    ...
    wordList.map((val, index) => {
      if (val[0].length >= MIN_SEARCH_LENGTH_NAMES) {
        ...
        search(columns, val[0], doSearchKeywordFuzzy, setGridHighlight, setGridHighlightOverride, false, true);
      }
      count++;
  ...
  const nameAssembler = (names_last, names_first_male, names_first_female) => {
    ...
    while(hits < 100000) {
      ...
      let result = search(columns, searchname, doSearchKeywordFuzzy, setGridHighlight, setGridHighlightOverride, false, false);
...
function search(columns, keyword, doSearchKeywordFuzzy, setGridHighlight, setGridHighlightOverride, doHighlight, doPrint) {
  let result = null;
  let best_gh = [];
  let best_gho = [];
  for (let currentColumn=0; currentColumn<columns.length; currentColumn++) {
